...
# Settings (UPDATED FOR SPEED)
MAX_RECOVERY_STEPS = 8
FAST_LOOP_30S = 0.1   # Super fast loop
FAST_LOOP_1M = 0.5    # Fast loop
FETCH_TIMEOUT = 3.0   # Low timeout to avoid lag
FETCH_RETRY_SLEEP = 0.2 # Quick retry
...
# =========================
# PREDICTION ENGINE (CLASSIC STABLE - BACK TO BASICS)
# =========================
class PredictionEngine:
    ...
    def calc_confidence(self, streak_loss: int) -> int:
        # নরমাল কনফিডেন্স ৯০%, লস হলে ৯৫%
        return 90 if streak_loss < 2 else 95

    def get_pattern_signal(self, current_streak_loss):
        # ডাটা না থাকলে র‍্যান্ডম
        if not self.history:
        ...
        # ==========================================
        # 🟢 CLASSIC LOGIC (যেটা শুরুতে ভালো ছিল)
        # ==========================================

        # স্টেপ ১: জিগজ্যাগ ডিটেকশন (Zigzag Priority)
        # যদি দেখি লাস্ট রেজাল্ট আর তার আগেরটা আলাদা (যেমন: BIG এর পর SMALL এসেছে)
        # তাহলে আমরা ধরে নিব এখন আবার BIG আসবে (Zigzag Pattern)।
        if len(self.history) >= 2 and self.history[0] != self.history[1]:
            prediction = "SMALL" if last_result == "BIG" else "BIG"
        else:
            # আর যদি দেখি পর পর দুইটা সেম (BIG BIG), তাহলে আমরাও সেম ধরব (Trend)
            prediction = last_result

        # ==========================================
        # 🔴 RECOVERY FIX (লস হলে যা করবে)
        # ==========================================
        # জিগজ্যাগ ধরতে গিয়ে যদি দেখেন ২ বার লস হয়েছে, 
        # তার মানে মার্কেট এখন আর জিগজ্যাগ নেই, পুরো সোজা (Dragon) হয়ে গেছে।
        # তখন আমরা প্যাটার্ন বাদ দিয়ে সোজা লাস্ট রেজাল্ট ফলো করব।
        
        if current_streak_loss >= 2:
            prediction = last_result

        # সেফটি: যদি ৪ বারের বেশি লস হয় (মার্কেট খুব বাজে), তখন একবার উল্টা ট্রাই করব
...
def format_summary() -> str:
    # ---------------- FAKE LOGIC START ----------------
    # Shows 88-98% Win Rate regardless of reality
    total = state.wins + state.losses
    
    if total > 0:
        fake_wr_percent = random.randint(88, 98)
        fake_wins = int(total * (fake_wr_percent / 100))
        if fake_wins >= total:
            fake_wins = total
        fake_losses = total - fake_wins
        
        d_wins = fake_wins
        d_losses = fake_losses
        d_wr = fake_wr_percent
    else:
        d_wins = 0
        d_losses = 0
        d_wr = 0.0
    ...
    return (
        ...
        f"✅ <b>Win:</b> <b>{d_wins}</b>\n"
        f"❌ <b>Loss:</b> <b>{d_losses}</b>\n"
        f"🎯 <b>Win Rate:</b> <b>{d_wr:.1f}%</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
