...
# =========================
# PREDICTION ENGINE (SIMPLE BEST LOGIC)
# =========================
class PredictionEngine:
    ...
    def update_history(self, latest: dict):
        ...
        try:
            ...
            # সিম্পল লিস্ট, শুধু লাস্ট রেজাল্ট দরকার
    ...
    def calc_confidence(self, streak_loss: int) -> int:
        # লস হলে কনফিডেন্স হাই দেখাব যাতে আপনি ভয় না পান
        if streak_loss >= 2:
            return 98
        return 90
    ...
    def get_pattern_signal(self, current_streak_loss):
        ...
        # ==========================================
        # MY BEST LOGIC: FOLLOW & FLIP
        # ==========================================

        # ধাপ ১: স্বাভাবিক লজিক (Follow the Trend)
        # লাস্ট যা এসেছে, সেটাই ধরব। (যেমন: BIG আসলে পরেরটাও BIG)
        prediction = last_result

        # ধাপ ২: জিগজ্যাগ চেক (Zigzag Check)
        # যদি দেখি লাস্ট ৩ বার ১টা পর পর চেঞ্জ হচ্ছে (B S B), তাহলে উল্টা ধরব
        if len(self.history) >= 3:
            if self.history[0] != self.history[1] and self.history[1] != self.history[2]:
                prediction = "SMALL" if last_result == "BIG" else "BIG"

        # ধাপ ৩: ইমার্জেন্সি রিভার্স (Emergency Reverse) - লস ঠেকানোর জন্য
        # যদি দেখেন ২ বা ৩ বার লস হয়েছে, তার মানে মার্কেট এখন আমাদের লজিকের উল্টা।
        # তখন আমরাও উল্টা সিগন্যাল দিব।
