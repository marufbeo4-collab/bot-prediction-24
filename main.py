...
# =========================
# PREDICTION ENGINE (SMART TREND DETECTOR)
# =========================
class PredictionEngine:
    ...
    def calc_confidence(self, streak_loss: int) -> int:
        return 92 + streak_loss  # লস বাড়লে কনফিডেন্স একটু বাড়িয়ে দেখাবে

    def get_pattern_signal(self, current_streak_loss):
        # ডাটা কম থাকলে লাস্ট রেজাল্ট ফলো করো
        ...
        # ==========================================
        # 🧠 SMART LOGIC: SCORE SYSTEM
        # ==========================================
        # আমরা দেখব লাস্ট ৬ বারের মধ্যে মার্কেট কী করেছে
        score = 0
        limit = min(len(self.history), 6)

        for i in range(limit - 1):
            if self.history[i] == self.history[i+1]:
                # যদি আগেরটার সাথে মিলে যায় (Trend/Dragon)
                score += 1
            else:
                # যদি চেঞ্জ হয় (Zigzag)
                score -= 1
        ...
        # যদি স্কোর পজিটিভ হয়, তার মানে মার্কেট এখন ট্রেন্ডে আছে (সেম কালার আসছে বেশি)
        if score > 0:
            prediction = last_result
            
        # যদি স্কোর নেগেটিভ হয়, তার মানে মার্কেট এখন জিগজ্যাগ (কালার পাল্টাচ্ছে বেশি)
        elif score < 0:
            prediction = "SMALL" if last_result == "BIG" else "BIG"
            
        # যদি সমান সমান হয়, তাহলে রিসেন্ট প্যাটার্ন দেখো (লাস্ট ২টা)
        else:
        ...
        # 🛑 LOSS PROTECTION (শুধুমাত্র ৫ স্টেপের পর)
        # যদি ৫ বার লস হয়, তার মানে আমাদের ক্যালকুলেশন ভুল হচ্ছে, তখন উল্টিয়ে দাও
